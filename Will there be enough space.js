...
function enough(cap, on, wait) {
   const residue = cap-on-wait
   if (residue>= 0) {
       return`0 # He can fit all ${wait} passengers`
   }else {
       //{Math.abs(residue)}-преобразует число в обсолутное значение (без минуса)
       return `${Math.abs(residue)} # He can't fit ${Math.abs(residue)} of the ${wait} waiting`
   }
}
